...
class LevDistTest(TestCase):
    ...
    def test_matching_words(self):
        """Test che controlla la distanza tra due parole sia <= 2"""
    ...
    def test_perfect_matching_words(self):
        """Test per il controllo che due parole siano perfettamente uguali"""
    ...
    def test_matching_words_custom_distance(self):
        """Test che controlla che la distanza tra due parole sia <= x"""
        self.assertEqual(lev_dist_custom_dist({"cane"}, {"canini"}, 4), True)

    def test_word_matches_in_words_list(self):
        """Test per il controllo di quale parola di una lista si avvicina ad una parola data"""
        self.assertEqual(lev_dist_str({'check-on'}, {'check-in', 'checkin', 'check in', 'arrivato', 'entrato', 'entro', 'presenza'}), 'check-on')

    def test_word_non_matches_in_words_list(self):
        """Test per il controllo di quale parola di una lista si avvicina ad una parola data"""
        self.assertEqual(lev_dist_str({'Pallone'}, {'check-in', 'checkin', 'check in', 'arrivato', 'entrato', 'entro', 'presenza'}), None)

    def test_word_best_match_in_words_list(self):
        """Test per il controllo della miglior parola in una lista"""
        self.assertEqual(lev_dist_str_correct_w({'check-on'}, {'check-in', 'checkin', 'check in', 'arrivato', 'entrato', 'entro', 'presenza'}), 'check-in')

    def test_word_non_match_in_words_list_at_all(self):
        """Test per il controllo della miglior parola in una lista"""
        self.assertEqual(lev_dist_str_correct_w({'Pallone'}, {'check-in', 'checkin', 'check in', 'arrivato', 'entrato', 'entro', 'presenza'}), None)
